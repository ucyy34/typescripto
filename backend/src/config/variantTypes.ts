...
const VARIANT_TYPES: Record<string, VariantType> = {
    beden: {
        label: 'Beden',
        icon: '📏',
 ...
 */
function getVariantTypes(): VariantTypeInfo[] {
 ...
 */
function getVariantValues(typeKey: string): string[] {
 ...
 */
function isValidVariantType(typeKey: string): boolean {
    return Object.prototype.hasOwnProperty.call(VARIANT_TYPES, typeKey);
}

/**
 * Generate SKU suffix from variant
 */
function generateSKUSuffix(colorName: string | null | undefined, variantType: string | null | undefined, variantValue: string | null | undefined): string {
    let suffix = '';
