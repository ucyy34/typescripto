...
const updateOrderStatusSchema = Joi.object({
    status: Joi.string()
        .valid('draft', 'pending', 'confirmed', 'cancelled') // Phase 4.1 Simplified Flow
        .required()
        .messages({
            'any.only': 'Invalid order status. Valid: draft, pending, confirmed, cancelled',
