...
const { ReturnRequest, Order, OrderItem, Store, User, Product } = require('../models');
...
class ReturnService {
  ...
  sanitizeText(input, maxLen = 1000) {
    return String(input || '')
      .replace(/[\u0000-\u001F\u007F]/g, '')
      .slice(0, maxLen)
      .trim();
   ...
   * @private
   */
  async validateReturnItems(items, order) {
    ...
    const orderItemsSubtotal = order.items.reduce((sum, oi) => sum + (Number(oi.price) * Number(oi.quantity)), 0);
   ...
   */
  async getReturnRequestById(returnId, userId, role = 'buyer') {
   ...
   */
  async getUserReturnRequests(userId, filters = {}) {
   ...
   * @returns {Promise<Object>} Returns and pagination
   */
  async getStoreReturnRequests(storeId, userId, filters = {}) {
   ...
   */
  async updateReturnStatus(returnId, userId, role, updateData) {
   ...
   */
  async cancelReturnRequest(returnId, userId, reason) {
