...
interface ShippingBreakdown {
    ...
    isFree: boolean;
    storeOwes?: number; // Added for cart calc
}

interface CalculateSupportParams {
    storeId: string;
    orderTotal: number;
...
interface StoreShippingResult extends ShippingBreakdown {
    storeId: string;
...
interface CartShippingSummary {
    totalShippingSupport: number;
    totalActualCost: number;
    totalPlatformCovered: number;
    totalStoreCovered: number;
    totalStoreOwes: number;
    storeChargePercentage: number;
    isFree: boolean;
    storeCount: number;
    appliedGlobalRule: string | null;
    maxShippingCap: number | null;
    capApplied: boolean;
    savedByCap: number;
}

interface CartShippingResult {
    stores: StoreShippingResult[];
    summary: CartShippingSummary;
}

interface ShippingReportOptions {
...
class ShippingSupportService {
     ...
     */
    async getPlatformDefaultCost(): Promise<number> {
        try {
            const cost = await PlatformSettings.getValue('default_shipping_cost', FALLBACK_SHIPPING_COST);
            return parseFloat(cost as string) || FALLBACK_SHIPPING_COST;
        } catch (error) {
     ...
     */
    async getStoreShippingChargePercentage(): Promise<number> {
        try {
            const percentage = await PlatformSettings.getValue('store_shipping_charge_percentage', 50);
            return Math.min(100, Math.max(0, parseFloat(percentage as string) || 50));
        } catch (error) {
     ...
     */
    async getMaxShippingCap(): Promise<number> {
        try {
            const cap = await PlatformSettings.getValue('max_shipping_cap', 0);
            return parseFloat(cap as string) || 0; // 0 means no cap
        } catch (error) {
    ...
    async calculateShippingSupport({ storeId, orderTotal, cartTotal = null, storeCount = 1, isNewCustomer = false }: CalculateSupportParams): Promise<ShippingBreakdown> {
        try {
            ...
            const platformRule = await this.findApplicablePlatformRule({
                cartTotal: (cartTotal !== null ? cartTotal : orderTotal),
    ...
    async calculateCartShipping(cartItems: any[], isNewCustomer: boolean = false): Promise<CartShippingResult> {
        try {
            ...
            const storeShipping: StoreShippingResult[] = [];
            ...
            for (const storeId of Object.keys(storeGroups)) {
                ...
                if (hasGlobalFreeShipping) {
                    ...
                    storeShipping.push({
                        ...
                        isFree: true,
                        storeOwes: 0 // Will be calc below
                    });
...
export = new ShippingSupportService();
