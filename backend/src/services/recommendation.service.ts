...
interface GetCartRecommendationsParams {
  cartItems?: CartItem[];
  userId?: string | null;
  limit?: number;
}
...
class RecommendationService {
  async getCartRecommendations({ cartItems = [], userId = null, limit = 6 }: GetCartRecommendationsParams): Promise<SerializedProduct[]> {
    const categories = [
      ...new Set(
        cartItems
    ...
    const cacheKey = this._cartCacheKey(userId, categories as string[], excludeIds as string[], limit);
  ...
  async getWishlistContextRecommendations(userId: string, limit: number = 6): Promise<SerializedProduct[]> {
  ...
  _cartCacheKey(userId: string | null, categories: string[], excludeIds: string[], limit: number): string {
  ...
  async recordOrderCompletion(event: OrderCompletedEvent | null): Promise<void> {
    if (!event) {
      return;
    }
...
export = new RecommendationService();
