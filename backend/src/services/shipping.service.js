...
const { Shipment, ShipmentItem, ShipmentEvent, Order } = require('../models');
...
async function getShipmentById(id) {
  ...
  if (process.env.SHIPPING_PERSIST === 'true') {
    try {
      const found = await Shipment.findByPk(id, { include: [{ model: ShipmentItem, as: 'items' }, { model: ShipmentEvent, as: 'events' }] });
...
async function cancelShipment(id) {
  ...
  if (process.env.SHIPPING_PERSIST === 'true') {
    try {
      const found = await Shipment.findByPk(id);
      if (found) {
...
async function track(trackingNumber) {
  ...
  if (process.env.SHIPPING_PERSIST === 'true') {
    try {
      const shipment = await Shipment.findOne({ where: { tracking_number: trackingNumber } });
      if (shipment) {
